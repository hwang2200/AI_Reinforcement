...
class QLearningAgent(ReinforcementAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        sampleQValue = self.computeValueFromQValues(nextState)
        updatedQValue = ((1-self.alpha) * oldQValue) + ((self.alpha) * (reward + (self.discount * sampleQValue)))
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        oldQValue = self.getQValue(state, action)
        sampleQValue = self.computeValueFromQValues(nextState)
        difference = (reward + (self.discount * sampleQValue)) - oldQValue
        
        for feature in features:
            updatedWeight = self.weights[feature] + (self.alpha * difference) * features[feature]
